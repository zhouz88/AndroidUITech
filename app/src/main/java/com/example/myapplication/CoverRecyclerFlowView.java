...
public class CoverRecyclerFlowView extends RecyclerView {
    public CoverRecyclerFlowView(Context context) {
        super(context);
        init();
    }

    public CoverRecyclerFlowView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public CoverRecyclerFlowView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        ...
        if (i == center) {
            order = childCount - 1;   //2
        } else if (i > center) {
            order = center + childCount - 1 - i;// 1
        } else {
            order = i; //0
        }
        return order;
