...
package com.example.myapplication.sourcecode.usage;
 ...
 * {@code state} {@link Bucket#STATE_ALL},
 * {@code defaultNetwork} {@link Bucket#DEFAULT_NETWORK_ALL},
 * {@code metered } {@link Bucket#METERED_ALL},
 * {@code roaming} {@link Bucket#ROAMING_ALL}.
 * <p />
 * <b>NOTE:</b> Calling {@link #querySummaryForDevice} or accessing stats for apps other than the
 * calling app requires the permission {@link Manifest.permission#PACKAGE_USAGE_STATS},
...
public class NetworkStatsManager {
     ...
     * tag {@link Bucket#TAG_NONE},
     * default network {@link Bucket#DEFAULT_NETWORK_ALL},
     * metered {@link Bucket#METERED_ALL},
    ...
    public Bucket querySummaryForDevice(@NonNull NetworkTemplate template,
        ...
        try {
            android.app.usage.NetworkStats stats =
                    new android.app.usage.NetworkStats(mContext, template, mFlags, startTime, endTime, mService);
    ...
    public Bucket querySummaryForUser(int networkType, @Nullable String subscriberId,
        ...
        android.app.usage.NetworkStats stats;
        stats = new android.app.usage.NetworkStats(mContext, template, mFlags, startTime, endTime, mService);
     ...
     */
    @WorkerThread
    public android.app.usage.NetworkStats querySummary(int networkType, @Nullable String subscriberId, long startTime,
    ...
    @SystemApi(client = MODULE_LIBRARIES)
    @WorkerThread
    public android.app.usage.NetworkStats querySummary(@NonNull NetworkTemplate template, long startTime,
    ...
    @SystemApi(client = MODULE_LIBRARIES)
    @WorkerThread
    public android.app.usage.NetworkStats queryTaggedSummary(@NonNull NetworkTemplate template, long startTime,
                                                             long endTime) throws SecurityException {
        Objects.requireNonNull(template);
        try {
            android.app.usage.NetworkStats result =
    ...
    @SystemApi(client = MODULE_LIBRARIES)
    @WorkerThread
    public android.app.usage.NetworkStats queryDetailsForDevice(@NonNull NetworkTemplate template,
                                                                long startTime, long endTime) {
        Objects.requireNonNull(template);
        try {
            final android.app.usage.NetworkStats result =
                    new android.app.usage.NetworkStats(mContext, template, mFlags, startTime, endTime, mService);
    ...
    @NonNull
    @WorkerThread
    public android.app.usage.NetworkStats queryDetailsForUid(int networkType, @Nullable String subscriberId,
                                                             long startTime, long endTime, int uid) throws SecurityException {
        return queryDetailsForUidTagState(networkType, subscriberId, startTime, endTime, uid,
            Bucket.TAG_NONE, Bucket.STATE_ALL);
    }

    /** @hide */
    @NonNull
    public android.app.usage.NetworkStats queryDetailsForUid(@NonNull NetworkTemplate template,
                                                             long startTime, long endTime, int uid) throws SecurityException {
     ...
     * defaultNetwork is going to be {@link Bucket#DEFAULT_NETWORK_ALL},
     * metered is going to be {@link Bucket#METERED_ALL}, and
     * roaming is going to be {@link Bucket#ROAMING_ALL}.
     ...
     * @param tag TAG of interest. Use {@link Bucket#TAG_NONE} for aggregated data
     *            across all the tags.
     * @return Statistics which is described above.
    ...
    @NonNull
    @WorkerThread
    public android.app.usage.NetworkStats queryDetailsForUidTag(int networkType, @Nullable String subscriberId,
                                                                long startTime, long endTime, int uid, int tag) throws SecurityException {
        return queryDetailsForUidTagState(networkType, subscriberId, startTime, endTime, uid,
            tag, Bucket.STATE_ALL);
    }
     ...
     * @param tag TAG of interest. Use {@link Bucket#TAG_NONE} for aggregated data
     *            across all the tags.
     * @param state state of interest. Use {@link Bucket#STATE_ALL} to aggregate
    ...
    @NonNull
    @WorkerThread
    public android.app.usage.NetworkStats queryDetailsForUidTagState(int networkType, @Nullable String subscriberId,
    ...
    @SystemApi(client = MODULE_LIBRARIES)
    @WorkerThread
    public android.app.usage.NetworkStats queryDetailsForUidTagState(@NonNull NetworkTemplate template,
                                                                     long startTime, long endTime, int uid, int tag, int state) throws SecurityException {
        Objects.requireNonNull(template);
        try {
            final android.app.usage.NetworkStats result = new android.app.usage.NetworkStats(
     ...
     * tag {@link Bucket#TAG_NONE},
     * default network is going to be {@link Bucket#DEFAULT_NETWORK_ALL},
     * metered is going to be {@link Bucket#METERED_ALL},
     * and roaming is going to be {@link Bucket#ROAMING_ALL}.
     * <p>Only includes buckets that atomically occur in the inclusive time range. Doesn't
     * interpolate across partial buckets. Since bucket length is in the order of hours, this
     * method cannot be used to measure data usage on a fine grained time scale.
     * This may take a long time, and apps should avoid calling this on their main thread.
     ...
     */
    @WorkerThread
    public android.app.usage.NetworkStats queryDetails(int networkType, @Nullable String subscriberId, long startTime,
                                                       long endTime) throws SecurityException, RemoteException {
        NetworkTemplate template;
        try {
            template = createTemplate(networkType, subscriberId);
    ...
    @SystemApi
    @RequiresPermission(anyOf = {
            Manifest.permission.NETWORK_STATS_PROVIDER,
            NetworkStack.PERMISSION_MAINLINE_NETWORK_STACK})
