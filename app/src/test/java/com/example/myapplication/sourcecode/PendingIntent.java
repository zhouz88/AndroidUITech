...
public final class PendingIntent implements Parcelable {
     ...
     * {@link Build.VERSION_CODES#R}, PendingIntents are assumed to
     * be mutable by default, unless {@link #FLAG_IMMUTABLE} is set. Starting
     * with {@link Build.VERSION_CODES#S}, it will be required to
     ...
     * {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, for apps that
     * target SDK {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE} or
    ...
    public static PendingIntent getActivityAsUser(Context context, int requestCode,
        ...
        try {
            ...
            IIntentSender target =
                android.app.ActivityManager.getService().getIntentSenderWithFeature(
    ...
    public void cancel() {
        try {
            android.app.ActivityManager.getService().cancelIntentSender(mTarget);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     * sending behavior.  May be built from an {@link android.app.ActivityOptions} to apply to an
     * activity start.
     *
     * @see #send(Context, int, Intent, android.app.PendingIntent.OnFinished, Handler, String)
     *
     ...
     * behavior.  May be built from an {@link android.app.ActivityOptions} to apply to an activity start.
     *
     * @throws CanceledException Throws CanceledException if the PendingIntent
     * is no longer allowing more intents to be sent through it.
     */
    ...
    public int sendAndReturnResult(Context context, int code, @Nullable Intent intent,
        ...
        try {
            ...
            if (context != null && isActivity()) {
                ...
                android.app.ActivityOptions activityOptions = options != null ? new android.app.ActivityOptions(options)
            ...
            return android.app.ActivityManager.getService().sendIntentSender(app,
    ...
    public boolean addCancelListener(@NonNull Executor executor,
        ...
        synchronized (mTarget) {
            ...
            if (wasEmpty) {
                boolean success;
                try {
                    success = android.app.ActivityManager.getService().registerIntentSenderCancelListenerEx(
                            mTarget, cli);
                } catch (RemoteException e) {
                    throw e.rethrowFromSystemServer();
                }
    ...
    public void removeCancelListener(@NonNull CancelListener cancelListener) {
        synchronized (mTarget) {
            ...
            if (cli.mCancelListeners.isEmpty()) {
                try {
                    android.app.ActivityManager.getService().unregisterIntentSenderCancelListener(mTarget,
                            cli);
                } catch (RemoteException e) {
                    throw e.rethrowFromSystemServer();
                }
     ...
     * {@link UserHandle#getUserHandleForUid(int)} to see which user will receive
     * the intent.
     *
    ...
    public @NonNull List<ResolveInfo> queryIntentComponents(@ResolveInfoFlagsBits int flags) {
        try {
            ParceledListSlice<ResolveInfo> parceledList = android.app.ActivityManager.getService()
    ...
    public boolean intentFilterEquals(@Nullable PendingIntent other) {
        ...
        try {
            return android.app.ActivityManager.getService().getIntentForIntentSender(other.mTarget)
