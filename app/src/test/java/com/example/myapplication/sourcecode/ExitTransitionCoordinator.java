...
public class ExitTransitionCoordinator extends ActivityTransitionCoordinator {
    ...
    protected void onReceiveResult(int resultCode, Bundle resultData) {
        switch (resultCode) {
            case MSG_SET_REMOTE_RECEIVER:
                stopCancel();
                mResultReceiver = resultData.getParcelable(KEY_REMOTE_RECEIVER, ResultReceiver.class);
                if (mIsCanceled) {
    ...
    public void startExit(android.app.Activity activity) {
        ...
        if (!mIsExitStarted) {
            ...
            android.app.ActivityOptions options = ActivityOptions.makeSceneTransitionAnimation(activity, this,
                    sharedElementNames, resultCode, data);
            activity.convertToTranslucent(new android.app.Activity.TranslucentConversionListener() {
                @Override
    ...
    /**
     * Called from {@link android.app.Activity#onStop()}
     */
