...
public class BackupHelperDispatcher {
    ...
    TreeMap<String, android.app.backup.BackupHelper> mHelpers = new TreeMap<String, android.app.backup.BackupHelper>();
    
    public BackupHelperDispatcher() {
    }

    public void addHelper(String keyPrefix, android.app.backup.BackupHelper helper) {
        mHelpers.put(keyPrefix, helper);
    }

    public void performBackup(ParcelFileDescriptor oldState, android.app.backup.BackupDataOutput data,
             ParcelFileDescriptor newState) throws IOException {
        // First, do the helpers that we've already done, since they're already in the state
        // file.
        int err;
        Header header = new Header();
        TreeMap<String, android.app.backup.BackupHelper> helpers = (TreeMap<String, android.app.backup.BackupHelper>)mHelpers.clone();
        ...
        for (Map.Entry<String, android.app.backup.BackupHelper> entry: helpers.entrySet()) {
    ...
    private void doOneBackup(ParcelFileDescriptor oldState, BackupDataOutput data,
            ParcelFileDescriptor newState, Header header, android.app.backup.BackupHelper helper)
            throws IOException {
        int err;
    ...
    public void performRestore(BackupDataInput input, int appVersionCode,
        ...
        android.app.backup.BackupDataInputStream stream = new BackupDataInputStream(input);
