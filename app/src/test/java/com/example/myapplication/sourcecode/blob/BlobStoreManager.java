 ...
 *
 * <p> Apps can publish and access a data blob using a {@link android.app.blob.BlobHandle} object which can
 * be created with {@link android.app.blob.BlobHandle#createWithSha256(byte[], CharSequence, long, String)}.
 * This {@link android.app.blob.BlobHandle} object encapsulates the following pieces of information used for
 * identifying the blobs:
 * <ul>
 *     <li> {@link android.app.blob.BlobHandle#getSha256Digest()}
 *     <li> {@link android.app.blob.BlobHandle#getLabel()}
 *     <li> {@link android.app.blob.BlobHandle#getExpiryTimeMillis()}
 *     <li> {@link android.app.blob.BlobHandle#getTag()}
 * </ul>
 * For two {@link android.app.blob.BlobHandle} objects to be considered identical, all these pieces of information
 * must be equal.
 *
 * <p> For contributing a new data blob, an app needs to create a session using
 * {@link BlobStoreManager#createSession(android.app.blob.BlobHandle)} and then open this session for writing using
 ...
 * {@link BlobStoreManager#createSession(android.app.blob.BlobHandle)}. Note that the session data is persisted
 ...
 * can be accessed using {@link BlobStoreManager#openBlob(android.app.blob.BlobHandle)}, assuming the caller
 * satisfies constraints of any of the access modes associated with that data blob. An app may
 * acquire a lease on a blob with {@link BlobStoreManager#acquireLease(android.app.blob.BlobHandle, int)} and
 * release the lease with {@link BlobStoreManager#releaseLease(android.app.blob.BlobHandle)}. A blob will not be
...
public class BlobStoreManager {
    ...
    /**
     * Create a new session using the given {@link android.app.blob.BlobHandle}, returning a unique id
     ...
     * {@link #acquireLease(android.app.blob.BlobHandle, int)} or one of it's other variants) after committing
     ...
     *
     * @param blobHandle the {@link android.app.blob.BlobHandle} identifier for which a new session
     *                   needs to be created.
     ...
     */
    public @IntRange(from = 1) long createSession(@NonNull android.app.blob.BlobHandle blobHandle)
            throws IOException {
        try {
     ...
     *
     * @param sessionId a unique id obtained via {@link #createSession(android.app.blob.BlobHandle)} that
     ...
     *
     * @param blobHandle the {@link android.app.blob.BlobHandle} representing the blob that the caller
     *                   wants to access.
     ...
     */
    public @NonNull ParcelFileDescriptor openBlob(@NonNull android.app.blob.BlobHandle blobHandle)
            throws IOException {
        try {
     ...
     * ({@link android.app.blob.BlobHandle#getExpiryTimeMillis()}) is elapsed.
     *
     ...
     *
     * @param blobHandle the {@link android.app.blob.BlobHandle} representing the blob that the caller wants to
     *                   acquire a lease for.
     ...
     *                              API is identical to {@link #acquireLease(android.app.blob.BlobHandle, int)}
     *                              where clients have to explicitly call
     *                              {@link #releaseLease(android.app.blob.BlobHandle)} when they don't
     ...
     *                                  {@link android.app.blob.BlobHandle#getExpiryTimeMillis()}.
     ...
     *                                acquire a lease.
     *
     * @see #acquireLease(android.app.blob.BlobHandle, int)
     * @see #acquireLease(android.app.blob.BlobHandle, CharSequence)
     */
    public void acquireLease(@NonNull android.app.blob.BlobHandle blobHandle, @IdRes int descriptionResId,
     ...
     * system that the caller wants the blob to be kept around.
     *
     * <p> This is a variant of {@link #acquireLease(android.app.blob.BlobHandle, int, long)} taking a
     ...
     * apps should prefer using {@link #acquireLease(android.app.blob.BlobHandle, int)} which will allow
     ...
     *                                acquire a lease.
     *
     * @see #acquireLease(android.app.blob.BlobHandle, int, long)
     * @see #acquireLease(android.app.blob.BlobHandle, CharSequence)
     */
    public void acquireLease(@NonNull android.app.blob.BlobHandle blobHandle, @NonNull CharSequence description,
                             @CurrentTimeMillisLong long leaseExpiryTimeMillis) throws IOException {
        try {
     ...
     * system that the caller wants the blob to be kept around.
     *
     * <p> This is similar to {@link #acquireLease(android.app.blob.BlobHandle, int, long)} except clients don't
     * have to specify the lease expiry time upfront using this API and need to explicitly
     * release the lease using {@link #releaseLease(android.app.blob.BlobHandle)} when they no longer like to keep
     * a blob around.
     *
     * <p> Any active leases will be automatically released when the blob's expiry time
     * ({@link android.app.blob.BlobHandle#getExpiryTimeMillis()}) is elapsed.
     ...
     *
     * @see #acquireLease(android.app.blob.BlobHandle, int, long)
     * @see #acquireLease(android.app.blob.BlobHandle, CharSequence, long)
     */
    public void acquireLease(@NonNull android.app.blob.BlobHandle blobHandle, @IdRes int descriptionResId)
            throws IOException {
     ...
     * system that the caller wants the blob to be kept around.
     *
     * <p> This is a variant of {@link #acquireLease(android.app.blob.BlobHandle, int)} taking a {@link CharSequence}
     ...
     * using {@link #acquireLease(android.app.blob.BlobHandle, int)} which will allow {@code description} to be
     * localized.
     *
     * <p> This is similar to {@link #acquireLease(android.app.blob.BlobHandle, CharSequence, long)} except clients
     ...
     * @throws IllegalArgumentException when {@code blobHandle} is invalid.
     */
    public void releaseLease(@NonNull android.app.blob.BlobHandle blobHandle) throws IOException {
        try {
    ...
    /**
     * Return the {@link android.app.blob.BlobHandle BlobHandles} corresponding to the data blobs that
     * the calling app currently has a lease on.
     *
     * @return a list of {@link android.app.blob.BlobHandle BlobHandles} that the caller has a lease on.
     */
    @NonNull
    public List<android.app.blob.BlobHandle> getLeasedBlobs() throws IOException {
        try {
     ...
     * {@link #acquireLease(android.app.blob.BlobHandle, int)} or one of it's other variants,
    ...
    @TestApi
    @Nullable
    public LeaseInfo getLeaseInfo(@NonNull android.app.blob.BlobHandle blobHandle) throws IOException {
        try {
     ...
     * {@link #createSession(android.app.blob.BlobHandle)} and once the session is created, clients can open and
    ...
    public static class Session implements Closeable {
         ...
         * data once it is committed using a {@link android.app.blob.BlobHandle} representing the blob.
         *
         ...
         * blob data once it is committed using a {@link android.app.blob.BlobHandle} representing the blob.
         *
         * <p> This needs to be called before committing the blob using
         * {@link #commit(Executor, Consumer)}.
         *
         ...
         * a {@link android.app.blob.BlobHandle} representing the blob.
         *
