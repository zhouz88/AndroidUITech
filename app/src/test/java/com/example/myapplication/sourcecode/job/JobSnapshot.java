...
public class JobSnapshot implements Parcelable {
    ...
    public JobSnapshot(android.app.job.JobInfo info, int satisfiedMask, boolean runnable) {
        mJob = info;
    ...
    public JobSnapshot(Parcel in) {
        mJob = android.app.job.JobInfo.CREATOR.createFromParcel(in);
    ...
    public boolean isChargingSatisfied() {
        return !mJob.isRequireCharging()
                || satisfied(android.app.job.JobInfo.CONSTRAINT_FLAG_CHARGING);
    }
    ...
    public boolean isBatteryNotLowSatisfied() {
        return !mJob.isRequireBatteryNotLow()
                || satisfied(android.app.job.JobInfo.CONSTRAINT_FLAG_BATTERY_NOT_LOW);
    }
    ...
    public boolean isRequireDeviceIdleSatisfied() {
        return !mJob.isRequireDeviceIdle()
                || satisfied(android.app.job.JobInfo.CONSTRAINT_FLAG_DEVICE_IDLE);
    }
