...
/**
 * Provides the interface through which a {@link android.app.backup.BackupAgent} writes entire files
...
public class FullBackupDataOutput {
    ...
    public FullBackupDataOutput(ParcelFileDescriptor fd, long quota) {
        mData = new android.app.backup.BackupDataOutput(fd.getFileDescriptor(), quota, 0);
        mQuota = quota;
        mTransportFlags = 0;
    }
    ...
    public FullBackupDataOutput(ParcelFileDescriptor fd, long quota, int transportFlags) {
        mData = new android.app.backup.BackupDataOutput(fd.getFileDescriptor(), quota, transportFlags);
        mQuota = quota;
        mTransportFlags = transportFlags;
    }
