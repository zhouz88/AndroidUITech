 ...
 * are programming a stage of the {@link android.graphics.Canvas} or {@link android.graphics.RenderNode} drawing pipeline.</b></p>
 *
 ...
 * {@link android.graphics.Canvas} drawing operation, Android (generally) assembles a single GPU fragment shader to
 ...
 *  <li>Logic for the {@link android.graphics.Shader}, {@link ColorFilter}, and {@link BlendMode} on the
 *  {@link Paint}.</li>
 *  <li>Color space conversion code, as part of Android’s color management.</li>
 * </ul>
 *
 * <p>A {@link RuntimeShader}, like other {@link android.graphics.Shader} types, effectively contributes a function
 ...
 * function receives as an input parameter the position of the pixel within the {@link android.graphics.Canvas} or
 ...
 * exception of any {@link android.graphics.Shader#getLocalMatrix(Matrix)} transformations applied to this shader.
 * Additionally, if the shader is invoked by another using {@link #setInputShader(String, android.graphics.Shader)},
 ...
 * {@link android.graphics.ColorSpace} for an AGSL shader is defined to be the color space of the destination, which
 ...
 * {@link android.graphics.ColorSpace.Named#LINEAR_EXTENDED_SRGB} color space:
 *
 * <pre class="prettyprint">
 ...
 * {@link RuntimeShader#setColorUniform(String, android.graphics.Color)} with the desired colors.  The value of a
 ...
 * sample from any {@link android.graphics.Shader}, which includes but is not limited to {@link android.graphics.BitmapShader}. To
 * make it clear that you are operating on an {@link android.graphics.Shader} object there is no "sample"
 ...
 * then be initialized and updated per frame by calling
 * {@link RuntimeShader#setInputShader(String, android.graphics.Shader)} with the desired shader. The value of a
 * shader uniform is undefined if it is declared in the AGSL shader but not initialized.</p>
 *
 * <p>Although most {@link android.graphics.BitmapShader}s contain colors that should be color managed, some contain
 ...
 * {@link #setInputBuffer(String, android.graphics.BitmapShader)}. Shaders initialized this way work much like
 * a regular {@link android.graphics.BitmapShader} (including filtering and tiling), with a few major differences:
 * <ul>
 ...
 * </ul>
 *
 * <p>In addition, when sampling from a {@link android.graphics.BitmapShader} be aware that the shader does not use
...
public class RuntimeShader extends android.graphics.Shader {
     ...
     */
    private ArrayMap<String, android.graphics.Shader> mShaderUniforms = new ArrayMap<>();
    ...
    public void setColorUniform(@NonNull String uniformName, @ColorInt int color) {
        setUniform(uniformName, android.graphics.Color.valueOf(color).getComponents(), true);
    }
    ...
    public void setColorUniform(@NonNull String uniformName, @ColorLong long color) {
        android.graphics.Color exSRGB = android.graphics.Color.valueOf(color).convert(android.graphics.ColorSpace.get(android.graphics.ColorSpace.Named.EXTENDED_SRGB));
     ...
     *              colorspace and will be available as a vec4 uniform in the program.
     */
    public void setColorUniform(@NonNull String uniformName, @NonNull android.graphics.Color color) {
        if (color == null) {
            throw new NullPointerException("The color parameter must not be null");
        }
        Color exSRGB = color.convert(android.graphics.ColorSpace.get(ColorSpace.Named.EXTENDED_SRGB));
        setUniform(uniformName, exSRGB.getComponents(), true);
    }
